...
<table id="example" class="datatable table table-striped">
  ...
  <tbody>
  	 <?php
  	  ...
  	  if($sel_duration=="Pending") 
				{ 

				 $sr_no=0;
		          $sql6 =mysqli_query($connection,"SELECT * From receipt WHERE  remaing_amt!=0 and d_name='$sub_user_id'")or die(mysqli_error($connection));
				 ...
				 <?php  if ($sud!=0) 
			{
				...
				{
					$sr_no=0;
		          $sql6 =mysqli_query($connection,"SELECT * From receipt WHERE  date(fees_due_date) BETWEEN  '$fromdate' AND '$todate' and d_name='$sub_user_id' ORDER BY fees_due_date DESC ")or die(mysqli_error($connection));
