...
<div class="col-lg-12" id="reportdata">
		...
		<table class="datatable table table-striped" id="example" cellspacing="0" width="100%">
			  ...
			  <?php 
			  	...
			  	if($utype=='admin')
			  	{
			  		$query = mysqli_query($connection,"SELECT * FROM receipt WHERE user_id='$user_id' order by fees_due_date DESC")or die(mysqli_error($connection));
      		 	}
      		 	else{
			  		$query = mysqli_query($connection,"SELECT * FROM receipt WHERE user_id='$sub_id' order by fees_due_date DESC")or die(mysqli_error($connection));
      		 	}
