...
if($count>0)
{
  $sql_query = mysqli_query($connection,"UPDATE student SET `name`='$sname',`contact_no`='$mob_no',`email`='$semail',`dob`='$dob',`tmp_address`='$tmp_add',`gender`='$gender',`degree`='$degree',`faculty`='$faculty',`pass_year`='$pyear',`coll_name`='$coll_name',`source_type`='$source_type',`course_list`='$course_list',`course_fee`='$course_fee',`type`='Crm',`generated_by`='$sub_id',`sub_user_id`='$sub_id',`cur_date`='$cur_date' WHERE id='$s_id'")or die(mysqli_error($connection));
    ...
    $sql_query = mysqli_query($connection,"INSERT INTO `student`(`name`, `contact_no`, `email`, `dob`, `tmp_address`,`gender`, `degree`, `faculty`, `pass_year`, `coll_name`, `source_type`,`course_list`,`course_fee`,`type`,`generated_by`,`user_id`,`sub_user_id`,`cur_date`,`d_name`) VALUES('$sname','$mob_no','$semail','$dob','$tmp_add','$gender','$degree','$faculty','$pyear','$coll_name','$source_type','$course_list','$course_fee','crm','$sub_id','$sess_id11','$sub_id','$cur_date','$d_id')")or die(mysqli_error($connection));
