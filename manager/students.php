...
<div class="col-md-12">
  <h4>Recent Student</h4>
     ...
     if($utype=='admin')
      {
          ...
          $qry=mysqli_query($connection,"SELECT * FROM student ")or die(mysqli_error($connection));
          }else{
            $qry=mysqli_query($connection,"SELECT * FROM student ")or die(mysqli_error($connection));
          }
     ...
     if($utype=='admin')
      {
        $qry1=mysqli_query($connection,"SELECT SUM(gross_amount) as Total, SUM(due_amt) as Due FROM common WHERE  student_id='$c_id' and type='Invoice'  ")or die(mysqli_error($connection));
      }
      else
      {
        $qry1=mysqli_query($connection,"SELECT SUM(gross_amount) as Total, SUM(due_amt) as Due FROM common WHERE  student_id='$c_id' and type='Invoice'  ")or die(mysqli_error($connection));
