      ...
      <div class="btn-group col-md-3" >
       ...
       <input type="hidden" class="form-control" name="tax_amt" id="tax_amt" placeholder="Tax Amount " onkeyup="taxoncalculation();">
		...
		<select class="form-control" name="sel_mode" id="sel_mode" required="">
			...
			if($utype=='admin')
			{
				$sql = mysqli_query($connection,"SELECT * FROM mode ")or die(mysqli_error($connection));
			}
			else{
				$sql = mysqli_query($connection,"SELECT * FROM mode ")or die(mysqli_error($connection));
			}
...
<div class="col-md-12">
  <h4>Recent Expenses</h4>
      ...
      <thead>
        ...
        <tbody>
        <?php
        $sql12 = mysqli_query($connection,"SELECT * FROM add_expense ")or die(mysqli_error($connection));
