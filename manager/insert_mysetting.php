...
if(!empty($old_pass)) 
  {
	 // $sql = mysql_query("UPDATE `user` SET `comp_name`='$cname',`name`='$name',`username`='$txt_email',`password`='$new_pass',`updated_at`='$dt' WHERE `id`='$id'")or die(mysql_error($connection));
		$sql = mysqli_query($connection,"UPDATE `user_profile` SET `name`='$name',`mobile`='$txt_cont',`email`='$txt_email',`password`='$new_pass' WHERE `id`='$id'")or die(mysqli_error($connection));
