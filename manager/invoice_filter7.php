  ...
  <tbody>
        <?php
		     if($sel_duration=="Pending")
				{ 
 $sql7 =mysqli_query($connection,"SELECT * From receipt WHERE  remaing_amt!=0  order by  id desc")or die(mysqli_error($connection));
  while($row7=mysqli_fetch_array($sql7))
  {
  $paid_amt=$row7['paid_amt'];
}
				 $sr_no=0;
		          $sql6 =mysqli_query($connection,"SELECT * From receipt WHERE  remaing_amt!=0")or die(mysqli_error($connection));
