...
if($utype=='user')
{
$run1 = mysqli_query($connection,"SELECT Distinct(`student_id`), MAX(`sche_date`),`generated_by`,`sche_time`,`sche_type`,`user_id`,`sub_user_id` FROM generate_log where `status`='active' and `d_name`='$dep'  GROUP BY  `student_id` HAVING  `user_id`='$sub_id' and MAX(`sche_date`)<'$cur_date' ")or die(mysqli_error($connection));
}else
{
$run1 = mysqli_query($connection,"SELECT Distinct(`student_id`), MAX(`sche_date`),`generated_by`,`sche_time`,`sche_type`,`user_id`,`sub_user_id` FROM generate_log where `status`='active' and `d_name`='$dep' GROUP BY  `student_id` HAVING `user_id`='$sub_id' and MAX(`sche_date`)<'$cur_date' ")or die(mysqli_error($connection));
}
