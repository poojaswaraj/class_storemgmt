	...
	<div class="col-lg-12">
			...
			<table class="datatable table table-striped" id="example" cellspacing="0" width="100%">
			  ...
			  <?php 
			  	...
			  	if($utype=='admin')
			  	{
      		 		$query = mysqli_query($connection,"SELECT * FROM receipt WHERE user_id='$user_id' and TIMESTAMPDIFF(day,NOW(),fees_due_date) BETWEEN 1 AND 4")or die(mysqli_error($connection));
      		 	}
      		 	else{
      		 		$query = mysqli_query($connection,"SELECT * FROM receipt WHERE user_id='$user_id' and TIMESTAMPDIFF(day,NOW(),fees_due_date) BETWEEN 1 AND 4")or die(mysqli_error($connection));
      		 	}
