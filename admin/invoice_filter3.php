...
<div class="col-md-1" id="in_1">
		<div class="small-box bg-aqua">
	        <div class="inner">
	        	          <?php 
$sql=mysqli_query($connection,"SELECT count(id) as b from receipt where  date(cur_date) BETWEEN  '$fromdate' AND '$todate' AND user_id='$user_id' and payment_mode='$type'")or die(mysqli_error($connection));
  ...
  <tbody>
        <?php
			if($fromdate!='' && $todate!='')
				{
					$sr_no=0;
		          $sql1 =mysqli_query($connection,"SELECT * From receipt WHERE  date(cur_date) BETWEEN  '$fromdate' AND '$todate' AND user_id='$user_id' AND payment_mode='$type' ORDER BY cur_date DESC ")or die(mysqli_error($connection));
