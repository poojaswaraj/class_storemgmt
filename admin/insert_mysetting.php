...
if(!empty($old_pass))
  {
	 // $sql = mysql_query("UPDATE `user` SET `comp_name`='$cname',`name`='$name',`username`='$txt_email',`password`='$new_pass',`updated_at`='$dt' WHERE `id`='$id'")or die(mysql_error($connection));
		$sql = mysqli_query($connection,"UPDATE `user` SET `comp_name`='$cname',`name`='$name',`username`='$txt_email',`password`='$new_pass',`contact`='$txt_cont',`updated_at`='$dt' WHERE `id`='$id'")or die(mysqli_error($connection));
