...
$query1 = mysqli_query($connection,"SELECT * FROM series WHERE id='$select_series' ");
            $row1=mysqli_fetch_array($query1);
                    $series_name=$row1['name'];

                     $str=substr($series_name, 0, strrpos($series_name, ' '));
                    $val=$row1['value'];
                      $ini =$row1['receipt']+1;
                     $a=$val.$ini;
...
$query = mysqli_query($connection,"UPDATE series SET receipt=receipt+1 WHERE id='$select_series'");  
	...
	$sql_query = mysqli_query($connection,"INSERT INTO `receipt`(`user_id`,`sub_user_id`, `sname`,`s_mobile`,`course_name`,`total_amt`,`paid_amt`,`remaing_amt`,`fees_due_date`,`c_name`,`c_mobile`,`book_code`,`book_issue`,`receipt_series`,`s_id`,`cur_date`,`number`,`invoice_id`,`payment_mode`,`paid_r_amount`,`fine`,`ser_no`,`paytm_name`,`d_name`)  
		VALUES ('$sub_id','$user_id','$name','$txt_mob_no','$txt_cname','$txt_total_amount','$txt_paid_amount','$txt_remaing_amount','$txt_date','$txt_cperson','$txt_contact','$book_code','$book_issue','$select_series','$student_id','$cur_date','$a','$invoice_id','$payment_mode','$txt_paid_receipt1','$fine','$ini','$paytm_name','$str')")or die(mysqli_error($connection));
	...
	if($payment_mode=='Paytm')
	{
		$sql_query5 = mysqli_query($connection,"UPDATE paytm SET cash=cash+'$txt_paid_amount' WHERE name='$paytm_name'");
			...
			$receipt_sum = mysqli_query($connection,"SELECT SUM(paid_amt) as a ,total_amt FROM receipt WHERE invoice_id='$invoice_id' and user_id='$sub_id'");
           $receipt_sum1=mysqli_fetch_array($receipt_sum);
           $pm=$receipt_sum1['a'];
           $tm=$receipt_sum1['total_amt']; 
 $query5 = mysqli_query($connection,"UPDATE common SET due_amt=$tm-$pm WHERE id='$invoice_id' and user_id='$sub_id'"); 
